        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '3rem', alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center' }}>
              {profile?.github && <a href={`https://${profile.github}`} target="_blank" rel="noopener noreferrer" className="btn btn-ghost btn-sm"><Github size={15} /> {t('social.github')}</a>}
              {(profile?.instagram || profile?.linkedin) && <a href={`https://${profile.instagram || profile.linkedin}`} target="_blank" rel="noopener noreferrer" className="btn btn-ghost btn-sm"><Instagram size={15} /> {t('social.instagram')}</a>}
              {profile?.telegram && <a href={`https://t.me/${profile.telegram.replace('@','')}`} target="_blank" rel="noopener noreferrer" className="btn btn-ghost btn-sm"><Send size={15} /> {t('social.telegram')}</a>}
            </div>
            {profile?.resume_url && (
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    {e.gpa && <div style={{ fontSize: '0.8rem', color: 'var(--color-text-3)', marginTop: 2 }}>{t('common.gpa')}: {e.gpa}</div>}
                  </div>
                </div>
                {e[`description_${lang}`] && (
