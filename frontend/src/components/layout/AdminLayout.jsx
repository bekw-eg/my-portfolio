...
function AdminNavItem({ path, label, icon: Icon, hint, exact, onNavigate, t }) {
  return (
    <NavLink
      to={path}
      end={exact}
    ...
    >
      ...
      <span className="admin-nav-item__content">
        ...
        <span className="admin-nav-item__hint">{hint}</span>
      </span>
    </NavLink>
  );
}
...
export default function AdminLayout() {
  ...
  const roleLabel = user?.role === 'superadmin' ? 'Super admin' : 'Portfolio admin';
    ...
    <div className="admin-shell">
      ...
      <aside className={`admin-sidebar ${sidebarOpen ? 'is-open' : ''}`}>
        <div className="admin-sidebar__top">
          <Link to="/" className="admin-brand">
            ...
            <span className="admin-brand__copy">
              ...
              <span className="admin-brand__subtitle">Portfolio CMS</span>
            </span>
          </Link>
        ...
        <div className="admin-sidebar__workspace">
          ...
          <div>
            <p className="admin-sidebar__workspace-title">Content operations</p>
            <p className="admin-sidebar__workspace-copy">
              A focused workspace for managing portfolio content, leads, and profile data.
            </p>
          </div>
        </div>
        ...
        <div className="admin-sidebar__section admin-sidebar__section--secondary">
          <p className="admin-sidebar__label">Shortcuts</p>
          <Link to="/" className="admin-nav-item" onClick={() => setSidebarOpen(false)}>
            <span className="admin-nav-item__icon">
              <Home size={17} />
            </span>
            <span className="admin-nav-item__content">
              <span className="admin-nav-item__label">Public site</span>
              <span className="admin-nav-item__hint">Review the published portfolio</span>
            </span>
          </Link>
        </div>
          ...
          <div className="admin-topbar__left">
            ...
            <div className="admin-topbar__context">
              ...
              <div className="admin-topbar__heading">
                ...
                <span className="admin-topbar__description">{activePage.description}</span>
              </div>
            </div>
          </div>
                ...
                <div className="admin-dropdown__menu">
                  {LANGS.map((item) => (
                    ...
                    >
                      ...
                      {lang === item.code && <span>Active</span>}
                    </button>
                  ))}
                </div>
              )}
